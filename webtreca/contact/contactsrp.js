document.addEventListener("DOMContentLoaded", function () {
    ...
    submitLink.addEventListener('click', function (event) {
        ...
        if (nameInput.value.trim() === "") {
            showError('nameError', "Molimo vas unesite vaše ime i prezime!");
            isValid = false;
        }

        // Provera email adrese
        if (emailInput.value.trim() === "" || !validateEmail(emailInput.value)) {
            showError('emailError', "Molimo vas unesite vašu email adresu!");
            isValid = false;
        }

        // Provera poruke
        if (messageInput.value.trim() === "") {
            showError('messageError', "Molimo vas unesite željenu poruku!");
            isValid = false;
        }

        // Ako je sve validno, odmah preusmeri na successful.html
        if (isValid) {
            window.location.href = '/successful/successfulsrp.html'; // Preusmeravanje na stranicu
        }
    });
